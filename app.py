...
app.config['CHUNK_SIZE'] = int(os.getenv('CHUNK_SIZE', 10485760))
app.config['UPLOAD_FOLDER'] = 'uploads'
app.config['BACKUP_FOLDER'] = 'backups'
app.config['LOG_FOLDER'] = 'logs'
allowed_origins_env = os.getenv('CORS_ORIGINS', 'http://localhost:3000,http://localhost:5000')
...
def init_db():
    with app.app_context():
        ...
        if not c.fetchone():
            ...
            ''', (admin_user, hashed))
            uid = c.lastrowid
            c.execute('INSERT INTO admins (user_id, role, created_by) VALUES (?, ?, ?)', (uid, 'SUPER_ADMIN', uid))
...
def superadmin_add_whitelist():
    ...
    db = get_db()
    c = db.cursor()
    c.execute('INSERT INTO whitelist (ip_address, added_by) VALUES (?, ?)', (ip, request.user['id']))
    db.commit()
...
def superadmin_add_blacklist():
    ...
    db = get_db()
    c = db.cursor()
    c.execute('INSERT INTO blacklist (ip_address, reason, added_by) VALUES (?, ?, ?)',
...
def superadmin_restore_backup(backup_id):
    ...
    telegram.send_admin(f"Database restored from backup {backup_id}")
...
def superadmin_update_settings():
    ...
    telegram.send_admin(f"Settings updated by @{request.user['username']}")
...
def toggle_maintenance():
    ...
    telegram.send_admin(f"Maintenance mode {status} by @{request.user['username']}")
...
def add_cors_headers(response):
    ...
    # If we're in dev, allow any origin; otherwise check against whitelist
    if os.getenv('FLASK_ENV') == 'development':
